...
task GenerateAnnotation {
    ...
    command {
        python3 /scripts/bed_annotate_script_hg19.py --annotation /reference_files/gencode.v19.annotation.gtf --bed ~{bed_to_annotate} --gene_bed ~{gene_bed} --exome_bed ~{exome_bed} ~{'--gene_list ' + gene_list} --output_prefix ~{output_prefix}
    }
    runtime {
        docker: "us.gcr.io/tag-team-160914/annotatebed:hg19"
...
task CountGeneBases {
    ...
    command {
            python3 /scripts/count_gene_base_hg19.py --gene_bed ~{gene_bed} --bed ~{bed_to_annotate} --grouped_by_gene ~{grouped_by_gene} ~{'--gene_list ' + gene_list} --output_prefix ~{output_prefix}
    }
    runtime {
        docker: "us.gcr.io/tag-team-160914/annotatebed:hg19"
        preemptible: preemptible
