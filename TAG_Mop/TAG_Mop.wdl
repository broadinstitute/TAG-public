...
workflow CleanupWithOptionalMop {
    ...
    output {
        ...
        File? mopped_files = select_first([ mop_with_submitter.mopped_files if runMop && defined(mop_with_submitter.mopped_files) else None, mop_without_submitter.mopped_files if runMop && defined(mop_without_submitter.mopped_files) else None ])
        Int? num_of_files_to_mop = select_first([ mop_with_submitter.num_of_files_to_mop if runMop && defined(mop_with_submitter.num_of_files_to_mop) else None, mop_without_submitter.num_of_files_to_mop if runMop && defined(mop_without_submitter.num_of_files_to_mop) else None ])
        String? total_size_to_mop = select_first([ mop_with_submitter.total_size_to_mop if runMop && defined(mop_with_submitter.total_size_to_mop) else None, mop_without_submitter.total_size_to_mop if runMop && defined(mop_without_submitter.total_size_to_mop) else None ])
    }
