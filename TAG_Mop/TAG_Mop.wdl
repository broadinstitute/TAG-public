...
workflow CleanupWithOptionalMop {
    ...
    output {
        ...
        File? mopped_files = select_first( if runMop then select_all([mop_with_submitter.mopped_files, mop_without_submitter.mopped_files]) else [])
        Int? num_of_files_to_mop = select_first( if runMop then select_all([mop_with_submitter.num_of_files_to_mop, mop_without_submitter.num_of_files_to_mop]) else [])
        String? total_size_to_mop = select_first( if runMop then select_all([mop_with_submitter.total_size_to_mop, mop_without_submitter.total_size_to_mop]) else [])
    }
