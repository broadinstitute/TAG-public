...
task pbSkerawQC {
    ...
    command <<<
        ...
        mkdir -p ./_skera_upload
        cp ~{skera_id}.skera.* ./_skera_upload/
        aws s3 cp ./_skera_upload/ ~{outdir}skera/ --recursive
        echo "Copying skera files completed!"

        echo "Copying plots to S3 path QC_plots..."
        mkdir -p ./_qc_upload
        cp ~{skera_id}*.png ./_qc_upload/
        aws s3 cp ./_qc_upload/ ~{outdir}QC_plots/ --recursive
...
task pbLimaBulk {
    ...
    command <<<
        ...
        echo "Copying output to S3 path provided..."
        mkdir -p ./_lima_upload
        cp ~{skera_id}*lima* ./_lima_upload/
        aws s3 cp ./_lima_upload/ ~{outdir}lima/ --recursive
        ...
        mkdir -p ./_refine_upload
        cp ~{skera_id}*refine* ./_refine_upload/
        aws s3 cp ./_refine_upload/ ~{outdir}refine/ --recursive
...
task pbSingleCell {
    ...
    command <<<
        ...
        echo "Copying output to S3 path provided..."
        mkdir -p ./_sc_lima_upload
        cp ~{sample_id}*lima* ./_sc_lima_upload/
        aws s3 cp ./_sc_lima_upload/ ~{outdir}lima/ --recursive
        echo "Copying lima files completed!"
        ...
        mkdir -p ./_tag_upload
        cp *.tagged* ./_tag_upload/
        aws s3 cp ./_tag_upload/ ~{outdir}tag/ --recursive
        echo "Copying extracted tagged reads completed!"

        echo "Uploading refined bams..."
        mkdir -p ./_sc_refine_upload
        cp *.refine* ./_sc_refine_upload/
        aws s3 cp ./_sc_refine_upload/ ~{outdir}refine/ --recursive
        echo "Copying refined reads completed!"

        echo "Uploading corrected bams..."
        mkdir -p ./_correct_upload
        cp *.corrected* ./_correct_upload/
        aws s3 cp ./_correct_upload/ ~{outdir}correct/ --recursive
...
task pbGroupdedup {
    ...
    command <<<
        ...
        mkdir -p ./_dedup_upload
        cp *.dedup* ./_dedup_upload/
        aws s3 cp ./_dedup_upload/ ~{outdir}groupdedup/ --recursive
        echo "Copying extracted tagged reads completed!"
