...
workflow SingleSampleCODEC_NVX_update {
    input {
        String sample_id
        File fastq1
        File fastq2
    ...
    scatter (split_index in range(num_parallel)) {
        ...
        call FilterOnReadLengthFromSAM  {
            input:
                sample_id = sample_id,
                bam_input = GroupReadByUMI.groupbyumi_bam
            }
        call FgbioCollapseReadFamilies {
            input:
                grouped_umi_bam = FilterOnReadLengthFromSAM.filtered_aligned_bam,
                sample_id = sample_id
    }
        ...
        call DuplexRecoveryMetrics {
            ...
            sample_id = sample_id,
            duplex_eval_bed = eval_genome_bed
        }
 ...
 output {
       ...
       # File? output_theoretical_sensitivity = CollectSelectionMetrics.output_theoretical_sensitivity
        ...
        File? germline_variants = CSS_SFC_ErrorMetrics.germline_variants
        ...
        File FilterMetrics  = FilterOnReadLengthFromSAM.filter_metrics
...
task FilterOnReadLengthFromSAM {
    input {
        File bam_input
        Int mem = 16
        Int? extra_disk
    ...
    String kept_bam_name = "${sample_id}.filtered_on_read_length.bam"
    String filtered_bam_name = "${sample_id}.filtered_out_read_length.bam"
    String output_filter_metrics = "${sample_id}.filter_metrics.tsv"

    command <<<
        set -euo pipefail

        # Convert BAM to SAM once
        samtools view -h ~{bam_input} > input.sam

        # Split SAM into kept and filtered
        awk -v sample="~{sample_id}" \
            -v metrics="~{output_filter_metrics}" '
            BEGIN { OFS="\t"; total=0; keep=0 }
            ...
            END{
                drop = total - keep
                frac = (total > 0 ? drop / total : 0)
                printf("%s\t%d\t%d\t%d\t%.6g\n",
                    sample, total, keep, drop, frac) >> metrics
            }
        ' input.sam

        # Convert back to BAM
        samtools view -b kept.sam -o ~{kept_bam_name}
        samtools view -b filtered.sam -o ~{filtered_bam_name}
    >>>

    output {
        File filtered_aligned_bam = kept_bam_name
        File filtered_out_bam = filtered_bam_name
        File filter_metrics = output_filter_metrics
    }
...
task CSS_SFC_ErrorMetrics {
    ...
    command {
        /CODECsuite/build/codec call -b ~{ConsensusAlignedBam} \
            ...
            -V ~{select_first([germline_vcf, population_based_vcf])} \
            -x 6 \
            -c 4 \
            -5 \
            -g 30 \
            ...
            ~{if defined(germline_vcf) then "-k " + sample_id + ".germline_variants.txt" else ""}
    }
    ...
    output {
        ...
        File? germline_variants = "~{sample_id}.germline_variants.txt"
    }
