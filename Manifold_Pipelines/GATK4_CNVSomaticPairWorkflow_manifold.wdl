...
########################################################################################################
# Tasks inlined from cnv_common_tasks.wdl (GATK 4.2.0.0) for Manifold/AWS.
# The `parameter_meta { bam/bam_idx: localization_optional }` blocks were removed so that BAM inputs
# are localized to the worker instead of streamed as raw s3:// URIs (GATK has no NIO "s3" provider).
########################################################################################################
