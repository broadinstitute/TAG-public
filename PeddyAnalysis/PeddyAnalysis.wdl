            ...
            for i, row in fam_file_df.iterrows():
                ...
                if sample_id in sample_info:
                    ...
                    if info['pedigree'] == 'Proband':
                        # Check for the presence of "Mother" or "Father" in pedigree
                        father_id_row = known_trio_info_df[
                            (known_trio_info_df['sidr_family_id'] == info['sidr_family_id']) &
                            (known_trio_info_df['pedigree'].str.contains('Father', case=False))
                        ]
                        mother_id_row = known_trio_info_df[
                            (known_trio_info_df['sidr_family_id'] == info['sidr_family_id']) &
                            (known_trio_info_df['pedigree'].str.contains('Mother', case=False))
                        ]
                        
                        # Handle the case where only one parent is present
                        father_id = '0'
                        mother_id = '0'
