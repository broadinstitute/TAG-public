...
task GenerateAnnotation {
    ...
    command {
        python3 /scripts/bed_annotate_script_hg38.py --annotation /reference_files/gencode.v43.annotation.gtf --bed ~{bed_to_annotate} --gene_bed ~{gene_bed} --exome_bed ~{exome_bed} ~{'--gene_list ' + gene_list} --output_prefix ~{output_prefix}
    }
    runtime {
        docker: "us.gcr.io/tag-team-160914/annotatebed:hg38"
...
task CountGeneBases {
    ...
    command {
            python3 /scripts/count_gene_base_hg38.py --gene_bed ~{gene_bed} --bed ~{bed_to_annotate} --grouped_by_gene ~{grouped_by_gene} ~{'--gene_list ' + gene_list} --output_prefix ~{output_prefix}
    }
    runtime {
        docker: "us.gcr.io/tag-team-160914/annotatebed:hg38"
        preemptible: preemptible
...
task summarize_coverage {
    ...
    command {
    python3 /scripts/summarize_coverage.hg38.py "${sep=' ' coverage_outputs}"
    }
    output{
    File samtools_coverage_summary = 'samtools_coverage_summary.txt'
    }
    runtime {
    	docker: "us.gcr.io/tag-team-160914/integrate_annotation:hg38"
        memory: memory_gb + "GB"
        disks: "local-disk " + disk_size + " HDD"
    }
}
